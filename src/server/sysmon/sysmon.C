...
#include "templates/system-monitor.h"
#include "sysmon.h"
...
std::unique_ptr<WWidget> SysMon::layout() {
  ...
  std::string htmlData;
  if (!readFile("templates/system-monitor.txt", htmlData)) {
      htmlData = std::string(reinterpret_cast<const char*>(SYSTEM_MONITOR_TPL), SYSTEM_MONITOR_TPL_SIZE);
      if (htmlData.empty())
        return container;
  }
  ...
  pimpl->CpuUsageModel->setHeaderData(0, "system-monitor-cpu-percents-user");
  pimpl->CpuUsageModel->setHeaderData(1, "system-monitor-cpu-percents-kernel");
  pimpl->CpuUsageModel->setHeaderData(2, "system-monitor-cpu-percents-idle");
  pimpl->CpuUsageModel->setHeaderData(3, "system-monitor-cpu-percents-iowait");
  pimpl->CpuUsageModel->setHeaderData(4, "system-monitor-cpu-percents-swap");
  pimpl->CpuUsageModel->setHeaderData(5, "system-monitor-cpu-percents-nice");
  pimpl->CpuUsageModel->setHeaderData(6, WString("-"));

  /// Memory Model
  pimpl->MemoryUsageModel = std::make_shared<WStandardItemModel>(60, 5);
  pimpl->MemoryUsageModel->setHeaderData(0, "system-monitor-memory-stats-total");
  pimpl->MemoryUsageModel->setHeaderData(1, "system-monitor-memory-stats-free");
  pimpl->MemoryUsageModel->setHeaderData(2, "system-monitor-memory-stats-used");
  pimpl->MemoryUsageModel->setHeaderData(3, "system-monitor-memory-stats-cache");
  pimpl->MemoryUsageModel->setHeaderData(4, WString("-"));

  /// Swap Model
  pimpl->SwapUsageModel = std::make_shared<WStandardItemModel>(60, 4);
  pimpl->SwapUsageModel->setHeaderData(0, "system-monitor-swap-stats-total");
  pimpl->SwapUsageModel->setHeaderData(1, "system-monitor-swap-stats-used");
  pimpl->SwapUsageModel->setHeaderData(2, "system-monitor-swap-stats-free");
  pimpl->SwapUsageModel->setHeaderData(3, WString("-"));

  /// Virtual Memory Model
  pimpl->VirtualMemoryUsageModel = std::make_shared<WStandardItemModel>(60, 4);
  pimpl->VirtualMemoryUsageModel->setHeaderData(0, "system-monitor-virtual-memory-stats-total");
  pimpl->VirtualMemoryUsageModel->setHeaderData(1, "system-monitor-virtual-memory-stats-used");
  pimpl->VirtualMemoryUsageModel->setHeaderData(2, "system-monitor-virtual-memory-stats-free");
  ...
  tmpl->bindWidget("host-info-title",
                    std::make_unique<WText>(WString("<h4>{1}</h4>").arg("system-monitor-host-info")));
  tmpl->bindWidget("cpu-info-title",
                    std::make_unique<WText>(WString("<h4>{1}</h4>").arg("system-monitor-cpu-percents")));
  tmpl->bindWidget("memory-info-title",
                    std::make_unique<WText>(WString("<h4>{1}</h4>").arg("system-monitor-memory-stats")));
  tmpl->bindWidget("swap-info-title",
                    std::make_unique<WText>(WString("<h4>{1}</h4>").arg("system-monitor-swap-stats")));
  tmpl->bindWidget("virtual-memory-info-title",
                    std::make_unique<WText>(WString("<h4>{1}</h4>").arg("system-monitor-virtual-memory-stats")));
  tmpl->bindWidget("disk-info-title",
                    std::make_unique<WText>(WString("<h4>{1}</h4>").arg("system-monitor-disk-io-stats")));
  tmpl->bindWidget("network-info-title",
                    std::make_unique<WText>(WString("<h4>{1}</h4>").arg("system-monitor-network-io-stats")));

  return container;
}

SysMon::Impl::Impl() : Timer(nullptr), HostState {
  {sg_unknown_configuration, "system-monitor-host-info-host-state-unknown-configuration"},
  {sg_physical_host, "system-monitor-host-info-host-state-physical-host"},
  {sg_virtual_machine, "system-monitor-host-info-host-state-virtual-machine"},
  {sg_paravirtual_machine, "system-monitor-host-info-host-state-paravirtual-machine"},
  {sg_hardware_virtualized, "system-monitor-host-info-host-state-hardware-virtualized"}}
{
...
void SysMon::Impl::RefreshResourceUsage() {
  ...
  if ((hostInfo = sg_get_host_info(&hostInfoEntries)) != nullptr) {
      ...
      horizontalHostInfoTable->elementAt(0, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-os-name"));
      horizontalHostInfoTable->elementAt(0, 1)->addWidget(std::make_unique<WText>("system-monitor-host-info-os-release"));
      horizontalHostInfoTable->elementAt(0, 2)->addWidget(std::make_unique<WText>("system-monitor-host-info-os-version"));
      horizontalHostInfoTable->elementAt(0, 3)->addWidget(std::make_unique<WText>("system-monitor-host-info-platform"));
      horizontalHostInfoTable->elementAt(0, 4)->addWidget(std::make_unique<WText>("system-monitor-host-info-hostname"));
      horizontalHostInfoTable->elementAt(0, 5)->addWidget(std::make_unique<WText>("system-monitor-host-info-bitwidth"));
      horizontalHostInfoTable->elementAt(0, 6)->addWidget(std::make_unique<WText>("system-monitor-host-info-host-state"));
      horizontalHostInfoTable->elementAt(0, 7)->addWidget(std::make_unique<WText>("system-monitor-host-info-ncpus"));
      horizontalHostInfoTable->elementAt(0, 8)->addWidget(std::make_unique<WText>("system-monitor-host-info-maxcpus"));
      horizontalHostInfoTable->elementAt(0, 9)->addWidget(std::make_unique<WText>("system-monitor-host-info-uptime"));
      horizontalHostInfoTable->elementAt(0, 10)->addWidget(std::make_unique<WText>("system-monitor-host-info-systime"));

      verticalHostInfoTable->elementAt(0, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-os-name"));
      verticalHostInfoTable->elementAt(1, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-os-release"));
      verticalHostInfoTable->elementAt(2, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-os-version"));
      verticalHostInfoTable->elementAt(3, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-platform"));
      verticalHostInfoTable->elementAt(4, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-hostname"));
      verticalHostInfoTable->elementAt(5, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-bitwidth"));
      verticalHostInfoTable->elementAt(6, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-host-state"));
      verticalHostInfoTable->elementAt(7, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-ncpus"));
      verticalHostInfoTable->elementAt(8, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-maxcpus"));
      verticalHostInfoTable->elementAt(9, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-uptime"));
      verticalHostInfoTable->elementAt(10, 0)->addWidget(std::make_unique<WText>("system-monitor-host-info-systime"));

      std::string systime(dateTimeString(hostInfo->systime));

      horizontalHostInfoTable->elementAt(1, 0)->addWidget(
        std::make_unique<WText>(WString(hostInfo->os_name)));
      horizontalHostInfoTable->elementAt(1, 1)->addWidget(
        std::make_unique<WText>(WString(hostInfo->os_release)));
      horizontalHostInfoTable->elementAt(1, 2)->addWidget(
        std::make_unique<WText>(WString(hostInfo->os_version)));
      horizontalHostInfoTable->elementAt(1, 3)->addWidget(
        std::make_unique<WText>(WString(hostInfo->platform)));
      horizontalHostInfoTable->elementAt(1, 4)->addWidget(
        std::make_unique<WText>(WString(hostInfo->hostname)));
      ...
      verticalHostInfoTable->elementAt(0, 1)->addWidget(
        std::make_unique<WText>(WString(hostInfo->os_name)));
      verticalHostInfoTable->elementAt(1, 1)->addWidget(
        std::make_unique<WText>(WString(hostInfo->os_release)));
      verticalHostInfoTable->elementAt(2, 1)->addWidget(
        std::make_unique<WText>(WString(hostInfo->os_version)));
      verticalHostInfoTable->elementAt(3, 1)->addWidget(
        std::make_unique<WText>(WString(hostInfo->platform)));
      verticalHostInfoTable->elementAt(4, 1)->addWidget(
        std::make_unique<WText>(WString(hostInfo->hostname)));
  ...
  if ((memStats = sg_get_mem_stats(&memStatsEntries)) != nullptr) {
    ...
    MemoryUsageModel->setHeaderData(0, WString::fromUTF8("{1} {2}")
      .arg(calculateSize(memStats->total))
      .arg("system-monitor-memory-stats-total"));
    MemoryUsageModel->setHeaderData(1, WString::fromUTF8("{1} ({2}%) {3}")
      .arg(calculateSize(memStats->free))
      .arg((wformat(L"%.1f") % memoryInstant[Memory::Free]).str())
      .arg("system-monitor-memory-stats-free"));
    MemoryUsageModel->setHeaderData(2, WString::fromUTF8("{1} ({2}%) {3}")
      .arg((calculateSize(memStats->used)))
      .arg((wformat(L"%.1f") % memoryInstant[Memory::Used]).str())
      .arg("system-monitor-memory-stats-used"));
    MemoryUsageModel->setHeaderData(3, WString::fromUTF8("{1} ({2}%) {3}")
      .arg(calculateSize(memStats->cache))
      .arg((wformat(L"%.1f") % memoryInstant[Memory::Cache]).str())
      .arg("system-monitor-memory-stats-cache"));
  ...
  if ((swapStats = sg_get_swap_stats(&swapStatsEntries)) != nullptr) {
    ...
    SwapUsageModel->setHeaderData(0, WString::fromUTF8("{1} {2}")
      .arg(calculateSize(swapStats->total))
      .arg("system-monitor-swap-stats-total"));
    SwapUsageModel->setHeaderData(1, WString::fromUTF8("{1} ({2}%) {3}")
      .arg((calculateSize(swapStats->used)))
      .arg((wformat(L"%.1f") % (!std::isnan(swapInstant[Swap::Used]) ? swapInstant[Swap::Used] : 0.0)).str())
      .arg("system-monitor-swap-stats-used"));
    SwapUsageModel->setHeaderData(2, WString::fromUTF8("{1} ({2}%) {3}")
      .arg(calculateSize(swapStats->free))
      .arg((wformat(L"%.1f") % (!std::isnan(swapInstant[Swap::Free]) ? swapInstant[Swap::Free] : 0.0)).str())
      .arg("system-monitor-swap-stats-free"));
  ...
  if (memStatsEntries > 0 && swapStatsEntries > 0) {
    ...
    VirtualMemoryUsageModel->setHeaderData(0, WString::fromUTF8("{1} {2}")
      .arg(calculateSize(total))
      .arg("system-monitor-virtual-memory-stats-total"));
    VirtualMemoryUsageModel->setHeaderData(1, WString::fromUTF8("{1} ({2}%) {3}")
      .arg((calculateSize(used)))
      .arg((wformat(L"%.1f") % virtualMemoryInstant[VirtualMemory::Used]).str())
      .arg("system-monitor-virtual-memory-stats-used"));
    VirtualMemoryUsageModel->setHeaderData(2, WString::fromUTF8("{1} ({2}%) {3}")
      .arg(calculateSize(free))
      .arg((wformat(L"%.1f") % virtualMemoryInstant[VirtualMemory::Free]).str())
      .arg("system-monitor-virtual-memory-stats-free"));
  ...
  diskTable->elementAt(0, 0)->addWidget(std::make_unique<WText>("system-monitor-disk-io-stats-disk-name"));
  diskTable->elementAt(0, 1)->addWidget(std::make_unique<WText>("system-monitor-disk-io-stats-read-bytes"));
  diskTable->elementAt(0, 2)->addWidget(std::make_unique<WText>("system-monitor-disk-io-stats-write-bytes"));
  diskTable->elementAt(0, 3)->addWidget(std::make_unique<WText>("system-monitor-disk-io-stats-systime"));
  ...
  if ((diskIoStats = sg_get_disk_io_stats_diff(&diskIoStatsEntries)) != nullptr) {
    ...
    diskTable->elementAt(static_cast<int>(diskIoStatsEntries) + 1, 0)->addWidget(
      std::make_unique<WText>("system-monitor-disk-io-stats-total"));
  ...
  networkTable->elementAt(0, 0)->addWidget(std::make_unique<WText>("system-monitor-network-io-stats-interface_name"));
  networkTable->elementAt(0, 1)->addWidget(std::make_unique<WText>("system-monitor-network-io-stats-tx"));
  networkTable->elementAt(0, 2)->addWidget(std::make_unique<WText>("system-monitor-network-io-stats-rx"));
  networkTable->elementAt(0, 3)->addWidget(std::make_unique<WText>("system-monitor-network-io-stats-ipackets"));
  networkTable->elementAt(0, 4)->addWidget(std::make_unique<WText>("system-monitor-network-io-stats-opackets"));
  networkTable->elementAt(0, 5)->addWidget(std::make_unique<WText>("system-monitor-network-io-stats-ierrors"));
  networkTable->elementAt(0, 6)->addWidget(std::make_unique<WText>("system-monitor-network-io-stats-oerrors"));
  networkTable->elementAt(0, 7)->addWidget(std::make_unique<WText>("system-monitor-network-io-stats-collisions"));
  networkTable->elementAt(0, 8)->addWidget(std::make_unique<WText>("system-monitor-network-io-stats-systime"));
  ...
  if ((networkIoStats = sg_get_network_io_stats_diff(&networkIoStatsEntries)) != nullptr) {
    ...
    networkTable->elementAt(static_cast<int>(networkIoStatsEntries) + 1, 0)->addWidget(
                std::make_unique<WText>("system-monitor-network-io-stats-total"));
